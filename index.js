$(function() {
  ...
  // 给excel去重
  $('#uniqueData').change(function(parentEvent) {
    let files = parentEvent.target.files;
    ...
    fileReader.onload = function(childEvent) {
      ...
      uniqueData = handleUniqueData(getExcelList[0])
      
      downloadOneSheet(uniqueData, files);
    };
  ...
  function handleUniqueData(arr) {
    for (var index = 0; index < arr.length; index++) {
      for (var i = index + 1; i < arr.length; i++) {
        if (arr[i]['代码'] == arr[index]['代码']) {
          arr.splice(i, 1)
          i--
        }
      }
